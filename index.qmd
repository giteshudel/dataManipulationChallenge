...
When treated as strings, the comparison happens character by character.
    ...
    - Because 4 comes before 5 alphabetically, "04-11-2025" is considered "less than" "05-20-2024", which is wrong chronologically
...
print(f"Found {len(late_shipments)} shipments that are at least {late_threshold} days late out of {len(shipments_with_lateness)} total")
```
...
- Using how='left' in .merge() is often preferred because it guarantees that all rows from the "left" DataFrame are preserved in the merged result. how='left' is a common and safe default when the goal is to add information to an existing, primary dataset without losing any of its original records. 
This is crucial in many data analysis scenarios for several reasons:
...
- While merging with shipments_df:
    ...
    Each shipment with partID "part123" will get BOTH rows.
    As a Result, 1 shipment becomes 2 rows in the merged data.
...
- "How often do our customers place orders that span multiple product categories, and what are the operational implications of these cross-category orders?"
