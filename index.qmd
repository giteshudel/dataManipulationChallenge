...
> Answer to Question 1:
> ascending=True: This sorts data in ascending order.
...
>
>
> ascending=False: This sorts data in descending order.
...
> Answer to Q2:
> If there are duplicate `partID` values in the `product_line_df`, we can run into data integrity issues. In his scenario, merge will create multple rows for each shipment that matches those duplicate partIDs. Here is an example:
> product_line_df has :
...
> Answer to Q2:
> | Group by `'prodCategory'` alone | Group by `['shipID', 'prodCategory']` |
    | Can help defining service metrics aggregated 
